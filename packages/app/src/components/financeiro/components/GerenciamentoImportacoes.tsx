...
import { ImportExport, Refresh, GetApp, Settings } from "@material-ui/icons"
...
interface GerenciamentoImportacoesProps {
  importInfo: ImportInfo
  onRefresh: () => void
}

export const GerenciamentoImportacoes: React.FC<GerenciamentoImportacoesProps> = ({ importInfo, onRefresh }) => {
  const [showImportSection, setShowImportSection] = useState(true)

  // Calcular totais
  const totalRegistros = Object.values(importInfo).reduce((acc, curr) => acc + curr.registros, 0)
  const totalMemoria = Object.values(importInfo).reduce((acc, curr) => acc + curr.memoria, 0)
  ...
  const exportarRelatorio = () => {
    ...
    link.href = url
    link.download = `relatorio-importacao-${new Date().toLocaleDateString().replace(/\//g, "-")}.json`
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <ImportExport style={{ fontSize: 24 }} />
        ...
        <div style={{ display: "flex", gap: "12px" }}>
          ...
          <button
            ...
            title={showImportSection ? "Minimizar" : "Expandir"}
          >
            {showImportSection ? "−" : "+"}
          </button>
        </div>
